...
async function runBasicTests() {
    ...
    try {
        ...
        console.log('3️⃣ Reading existing users...');
        const existingUsers = await dbUtils.testUsers.getAll();
        ...
        const newUser = await dbUtils.testUsers.create(
            'Test User ' + Date.now(),
        ...
        console.log('5️⃣ Finding user by ID...');
        const foundUser = await dbUtils.testUsers.findById(newUser.id);
        console.log('✅ User found:', foundUser);
        console.log();

        // Test 6: Update user
        console.log('6️⃣ Updating user...');
        const updatedUser = await dbUtils.testUsers.update(
            newUser.id,
            'Updated ' + newUser.name,
            newUser.email
        );
        ...
        const userCount = await dbUtils.testUsers.count();
        ...
        const customResult = await query(
            'SELECT name, email, created_at FROM test_users WHERE created_at > $1 ORDER BY created_at DESC',
            [new Date(Date.now() - 24 * 60 * 60 * 1000)] // Last 24 hours
        );
        console.log('✅ Recent users:', customResult.rows.length);
        console.log();
